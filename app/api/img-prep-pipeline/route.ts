...
export async function POST(req: NextRequest) {
  try {
    ...
    const processImage = async (image: any) => {
      ...
      try {
        // Use modifiedGcsObjectUrl only if it exists, otherwise use originalGcsObjectUrl
        const imageUrl = image.modifiedGcsObjectUrl || image.originalGcsObjectUrl;
        const [signedUrl] = await userImgBucket.file(imageUrl.split('/').pop()!).getSignedUrl({
          version: 'v4',
          action: 'read',
        ...
        console.log(`\x1b[36m[img-prep-pipeline] Signed URL: ${signedUrl}\x1b[0m`);

        // we will also print the input
        console.log(`\x1b[36m[img-prep-pipeline] Input: ${JSON.stringify({ image_url: signedUrl, sync_mode: true }, null, 2)}\x1b[0m`);
        ...
        const fileName = `nobg_${image.uuid}_${image.sanitizedFileName}`;
        ...
        console.log(`\x1b[36m[img-prep-pipeline] Processed image uploaded to GCS: ${preprocessedUrl}\x1b[0m`);
        ...
        await db.run(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `, image.id, image.sanitizedFileName, fileName, preprocessedUrl, caption, "claude-3-haiku-20240307");
